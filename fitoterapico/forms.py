...
class FitoterapicoModelForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'preco': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'Preço em R$', 'min': '1.00', 'step': '0.01'}),
    ...
    def clean_preco(self):
        ...
        if preco is not None and preco < 1:
            raise forms.ValidationError('O preço deve ser maior que R$1,00.')
        return preco
        
    def clean_especie(self):
        especie = self.cleaned_data.get('especie')
        if especie and not re.match(r'^[A-Za-z\s]+$', especie):
            raise forms.ValidationError('A espécie deve conter apenas letras e espaços.')
        return especie